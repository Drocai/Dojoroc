...
import { Target, Check, Gift, Flame } from 'lucide-react';
import { themeFor } from '../lib/theme';
import { todaysQuests, questProgress, isComplete, isClaimed, claimable } from '../lib/quests';
import { streakMultiplier, streakBonusLabel } from '../lib/streak';
...
const DailyQuests = ({ quests, streak, accent = 'emerald', onClaim }) => {
  ...
  const reward = (q) => Math.round(q.xp * mult);
    ...
    <div className="hud bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {bonus && <span className="text-[10px] font-semibold text-amber-400 flex items-center gap-0.5" title={`${streak?.count}-day streak bonus`}><Flame size={11} /> {bonus} XP</span>}
          <span className="text-xs font-mono text-zinc-500">{doneCount}/{list.length}</span>
        </div>
      </div>
          ...
          return (
            <div key={q.id} className="bg-zinc-950 border border-zinc-800 rounded-2xl px-3.5 py-2.5">
              <div className="flex items-center justify-between gap-2 mb-1.5">
                ...
                {claimed ? (
                  <span className={`text-[11px] flex items-center gap-1 ${theme.text}`}><Check size={12} /> +{reward(q)}</span>
                ) : canClaim ? (
                  <button onClick={() => onClaim(q)} className={`text-[11px] px-2.5 py-1 rounded-lg ${theme.btn} text-white font-semibold flex items-center gap-1 animate-pulse`}>
                    <Gift size={11} /> Claim +{reward(q)}
                  </button>
                ) : (
                  <span className="text-[11px] font-mono text-zinc-500">{prog}/{q.goal} · {reward(q)} XP</span>
                )}
              </div>
